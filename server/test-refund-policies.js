...
function testRefundPolicies() {
  ...
  const testCases = [
    {
      ...
      refund_data: {
        ...
        amount_requested: 99,
        purchase_date: '2024-10-15', // 15 days ago
    ...
    },
    {
      ...
      refund_data: {
        ...
        purchase_date: '2024-09-15', // 45+ days ago
  ...
  // Policy summary
  console.log('📊 Refund Policy Summary:');
  console.log('   • Time-based eligibility periods vary by product type');
  console.log('   • Billing errors and duplicate charges always eligible');
  console.log('   • Processing fees may apply to certain product types');
  console.log('   • Partial refunds available for some products');

  console.log('\n✅ Refund policies verified!');
}
...
function processRefundRequest(user, refundData) {
  ...
  const eligible = timeEligible && conditionMet;
  
  // Special case for billing errors - always full refund
  if (reason === 'billing_error' || reason === 'duplicate_charge') {
    approvedAmount = parseFloat(amount_requested);
  }
  ...
  return {
    ...
    processing_fee: eligibilityCheck.processing_fee || 0,
    net_refund: eligible ? eligibilityCheck.approved_amount - (eligibilityCheck.processing_fee || 0) : 0,
